...
def precompute_rope_cache(head_dim, max_seq_len, theta=10000.0):
    """Precompute the RoPE cos/sin frequency caches, each of shape (max_seq_len, head_dim // 2)"""
    ...
    t = torch.arange(max_seq_len, dtype=torch.float) # (max_seq_len,)
    # freqs[m, i] = m * theta^(-2i / head_dim): the rotation angle of dimension pair i at position m
    freqs = torch.outer(t, inv_freq) # (max_seq_len, head_dim // 2)
    return freqs.cos(), freqs.sin()

def apply_rotary_emb(x, cos, sin):
    """Rotate x of shape (B, nh, T, hs) by the position angles in cos/sin of shape (T, hs // 2)"""
    x1, x2 = x.chunk(2, dim=-1) # each (B, nh, T, hs // 2); dims i and i + hs // 2 are rotated as a 2D pair
    cos = cos[None, None, :, :] # broadcast over the batch and head dimensions
    sin = sin[None, None, :, :]
    y1 = x1 * cos - x2 * sin
    y2 = x1 * sin + x2 * cos
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # precompute the RoPE cos/sin caches (constants; persistent=False keeps them out of the state_dict)
        head_dim = config.n_embd // config.n_head
        cos, sin = precompute_rope_cache(head_dim, config.block_size)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        sd_keys_hf = [k for k in sd_keys_hf if k != 'transformer.wpe.weight'] # we replaced wpe with RoPE, skip it
